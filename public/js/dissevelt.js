...
$.ajax({
	...
	success: function(data) {
		...
		textureLoader.load(imagen_fondo, function(texture) {
...
function init(texture) {
	...
	*/
	...
	fondo = new THREE.Mesh(new THREE.SphereBufferGeometry(1000, 32, 16), new THREE.MeshBasicMaterial({ map: texture }));
	fondo.scale.x = -1;
	scene.add(fondo);
	...
	var geometry = new THREE.CylinderGeometry(0, 40, 80, 4, 1);
	...
	for (var i = 0; i < notas.length; i++) {
		...
		mesh.position.x = (no_al[i] - 0.5) * 1000;
		mesh.position.y = (no_al[i + 1] - 0.5) * 1000;
		mesh.position.z = (no_al[i + 2] - 0.5) * 1000;


		mesh.rotation.x = no_al[i + 3] * 2 * Math.PI;
		mesh.rotation.y = no_al[i + 4] * 2 * Math.PI;
		mesh.rotation.z = no_al[i + 5] * 2 * Math.PI;
		/*
...
/*
var fondos = ["img/mario1.jpg", "img/mario2.jpg", "img/mario3.jpg", "img/mario4.jpg"];
var contadorFondo = 1;
*/
function cambiarFondo() {
	//	contadorFondo = (contadorFondo + 1) % (fondos.length + 1);

	//textureLoader.load(fondos[contadorFondo], function(texture) {
	textureLoader.load(imagen_fondo, function(texture) {
		texture.mapping = THREE.UVMapping;
		fondo.material.map = texture;
		fondo.material.map.needsUpdate = true;
...
function render(time) {
	...
	for (var i = 0; i < trian.length; i++) {
		...
		trian[i].text.position.x = trian[i].position.x + 150;
	...
	*/
	...
	cube.position.x = Math.cos(time * 1) * 150;
	cube.position.y = Math.sin(time * 1) * 150;
	cube.position.z = Math.sin(time * 1) * 150;
